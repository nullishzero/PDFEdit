...
static QString trimmedCodeLine( const QString& t )
{
    ...
    */
    k = trimmed.indexOf( QString::fromLatin1("//") );
    if ( k != -1 )
	trimmed.truncate( k );
...
static bool bottomLineStartsInCComment()
{
    ...
    */
	...
	if ( (*p).contains(slashAster) || (*p).contains(asterSlash) ) {
	    QString trimmed = trimmedCodeLine( *p );

	    if ( trimmed.contains(slashAster) ) {
		return true;
	    } else if ( trimmed.contains(asterSlash) ) {
		return false;
	    }
	}
    }
...
{
    int k = yyLine->lastIndexOf( QString::fromLatin1("/*") );
    if ( k == -1 ) {
	/*
...
static bool isUnfinishedLine()
{
    ...
    if ( !QString(QString::fromLatin1("{};")).contains(lastCh) && !yyLine->endsWith(QString::fromLatin1("...")) ) {
	/*
...
static int indentForStandaloneLine()
{
	...
	if ( yyLine->endsWith(QString::fromLatin1(";")) || yyLine->count('{') > 0 ) {
	    /*
